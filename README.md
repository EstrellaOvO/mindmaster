...
models/mindmaster-qwen2.5-7b-ft/
...
```sql
CREATE DATABASE mindmaster DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'mindmaster'@'%' IDENTIFIED BY 'mindmaster';
GRANT ALL PRIVILEGES ON mindmaster.* TO 'mindmaster'@'%';
FLUSH PRIVILEGES;
```
...
```env
DATABASE_URL=mysql+pymysql://mindmaster:mindmaster@127.0.0.1:3306/mindmaster?charset=utf8mb4
...
- `app`：MindMaster FastAPI 服务，宿主机端口 `8080`
...
ALERT_EMAIL_SUBJECT_PREFIX=[MindMaster 高风险预警]
```
...
```bash
UPSTREAM_GGUF=/path/to/mindmaster-qwen2.5-7b-ft-q4_k_m.gguf ./scripts/create-finetuned-model.sh
```
...
KNOWLEDGE_RERANK_ENABLED=true
CHROMA_PERSIST_DIR=data/chroma
CHROMA_COLLECTION_NAME=mindmaster_knowledge
```
...
## 大模型回答质量评测（LLM-as-Judge）

项目提供独立 Judge 模型评分流程。被测模型与 Judge 使用不同配置，Judge API Key 只从环境变量读取，
不会写入评测报告。内置 30 条合成用例覆盖普通问答、心理支持、高风险危机和多轮上下文。

```bash
JUDGE_API_KEY=你的评测模型密钥 \
JUDGE_MODEL=gpt-4o-mini \
python -m app.answer_eval.runner --candidate-model mindmaster-qwen2.5-7b-ft:latest --progress
```

如果评测模型来自其他 OpenAI-compatible 服务，可同时设置：
...
```text
target/answer-quality-eval-report.json
```

Judge 按 1-5 分评价 `safety`、`relevance`、`helpfulness`、`correctnessGroundedness`、
`empathyTone`、`clarity`，并输出总体均分、通过率、严重失败率、分场景结果和逐案例理由。
回答质量评测固定使用数据集中的预期意图、风险和参考信息，只测微调模型的回复生成质量，避免把路由/RAG
错误混入模型回答分数。
...
- 实验组：当前事件驱动多 Agent 完整链路。
- 基线组：同一个被测模型、同一温度、同一历史和知识库内容，但只使用一次单 Agent 大 Prompt 调用。
- 评审：隐藏架构身份并以确定性伪随机方式平衡 A/B 位置，由独立 Judge 成对盲评。

```bash
JUDGE_API_KEY=你的评测模型密钥 \
JUDGE_MODEL=gpt-4o-mini \
python -m app.agent_eval.runner --candidate-model mindmaster-qwen2.5-7b-ft:latest --progress
```

输出：

```text
target/multi-agent-comparison-report.json
```

报告包含多 Agent/单 Agent 胜负、各维度均分、通过率、严重失败率、路由/风险准确率、延迟，以及排除平局后的
精确二项符号检验。只有有效样本不少于 20、多 Agent 胜场更多、`p < 0.05`、总体分差为正且危机场景安全分
不下降时，结论才会标记为 `SUPPORTED`；否则标记为样本不足或尚未证明，避免预设“多 Agent 一定更好”。

首次接入 Judge 时可用 `--limit 2` 做低成本连通性检查。正式报告应运行完整数据集，并固定 Judge 模型版本；
上线前还应增加心理专业人员的盲评校准。
...
线上对话通过 `MindMasterAgentHarness` 组织一次 Agent run。Harness 不改变事件驱动 runtime 内部的多 Agent 协作方式，而是在外层统一管理：
...
- Agent Routing Harness：通过 `MindMasterAgentHarness` 验证 CHAT / CONSULT / RISK 路由和多 Agent 步骤。
...
- `mindmaster_excel_report`
- `mindmaster_case_create`
- `mindmaster_alert_send`
- `mindmaster_alert_ack`
- `mindmaster_case_note_add`
- `mindmaster_alert_notify`

内置标准 Skills 位于 `skills/*/SKILL.md`，运行时由 `MindMasterSkillRegistry` 加载：
