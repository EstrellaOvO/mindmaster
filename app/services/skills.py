...
class MindMasterSkillRegistry:
    def __init__(self, root: Path | None = None):
        self.root = root or Path(__file__).resolve().parents[2] / "skills"

    def list_skills(self) -> list[MindMasterSkill]:
    ...
    def get_required(self, name: str) -> MindMasterSkill:
    ...
    def _load_skill_file(self, path: Path) -> MindMasterSkill:
        ...
        return MindMasterSkill(name=name.strip(), description=description.strip(), body=body.strip(), path=path, metadata=metadata)


class MindMasterSkillLibrary:
    @staticmethod
    def registry() -> MindMasterSkillRegistry:
        return MindMasterSkillRegistry()

    @staticmethod
    def list_skills() -> list[MindMasterSkill]:
        return MindMasterSkillLibrary.registry().list_skills()

    @staticmethod
    def status_items() -> list[dict]:
        return MindMasterSkillLibrary.registry().status_items()

    @staticmethod
    def response_skill_context(intent: IntentType, risk: RiskLevel, text: str) -> str:
        names = MindMasterSkillLibrary.response_skill_names(intent, risk, text)
        registry = MindMasterSkillLibrary.registry()
    ...
    def high_risk_safety_plan_prompt() -> str:
        return MindMasterSkillLibrary.registry().get_required("high_risk_safety_plan").prompt_context()

    @staticmethod
    def counselor_handoff_summary(report: PsychologicalReport, user: UserAccount | None) -> str:
        template = MindMasterSkillLibrary.registry().template_for("counselor_handoff_summary")
