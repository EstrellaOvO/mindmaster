...
from app.services.mcp_client import MindMasterMcpToolClient
...
class MindMasterAgentHarness:
    """Runtime harness for one MindMaster agent turn.
    ...
    async def dispatch_tools(self, tool_plan: AgentToolPlan) -> list[str]:
        ...
        if self.settings.tool_queue_enabled:
            ...
            return ["queued"]
        return await MindMasterMcpToolClient(self.settings).handle_report(tool_plan.report_id, tool_plan.risk_level)
