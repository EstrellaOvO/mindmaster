...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run MindMaster engineering harness checks.")
    parser.add_argument(
        "--suite",
        action="append",
...
def configure_environment() -> None:
    ...
    db_path = target_dir / "mindmaster-harness.sqlite3"
    ...
    os.environ["EXCEL_PATH"] = str((target_dir / "mindmaster-risk-ledger.xlsx").as_posix())
...
def run_agent_routing_harness(context: HarnessContext) -> dict:
    from app.agents.harness import MindMasterAgentHarness
    ...
    try:
        ...
        for case in cases:
            ...
            db.commit()
            db.refresh(session)
            result = MindMasterAgentHarness(db, context.settings).run(
                user,
...
def run_standard_skills_harness(context: HarnessContext) -> dict:
    ...
    from app.services.skills import MindMasterSkillLibrary
    ...
    skills = MindMasterSkillLibrary.list_skills()
    names = {skill.name for skill in skills}
    missing = sorted(expected - names)
    expect(not missing, f"missing standard skills: {missing}")

    statuses = MindMasterSkillLibrary.status_items()
    ...
    selected_names = MindMasterSkillLibrary.response_skill_names(
        IntentType.CONSULT,
        RiskLevel.LOW,
        "我最近焦虑、失眠，考试压力也很大。",
    )
    ...
    context_text = MindMasterSkillLibrary.response_skill_context(
        IntentType.CONSULT,
        RiskLevel.LOW,
        "我最近焦虑、失眠，考试压力也很大。",
    )
    ...
    high_risk_names = MindMasterSkillLibrary.response_skill_names(
        IntentType.RISK,
        RiskLevel.HIGH,
        "我不想活了。",
    )
    ...
    handoff = MindMasterSkillLibrary.counselor_handoff_summary(report, user)
...
def print_report(report: dict) -> None:
    print("MindMaster Engineering Harness")
