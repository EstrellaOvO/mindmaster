...
MODEL_NAME="${FINETUNED_MODEL_NAME:-mindmaster-qwen2.5-7b-ft:latest}"
MODEL_DIR="${FINETUNED_MODEL_DIR:-$ROOT_DIR/models/mindmaster-qwen2.5-7b-ft}"
GGUF_FILE="${FINETUNED_MODEL_FILE:-mindmaster-qwen2.5-7b-ft-q4_k_m.gguf}"
...
if [ ! -f "$MODEL_DIR/$GGUF_FILE" ]; then
  if [ -n "$UPSTREAM_GGUF" ] && [ -f "$UPSTREAM_GGUF" ]; then
    echo "Linking GGUF from upstream MindMaster project..."
...
echo "Run MindMaster Python with: AI_PROVIDER=ollama ./scripts/run-dev.sh"
